...
class ElasticSearchService {
	...
	public TableResultSet executeQuery2(logApplication, logLevel, logSearch, pageSize, skip, sortField, sortOrder) {

		def resultSet = new TableResultSet();
		...
		searchResponse.hits.each {SearchHit hit ->
			def sourceMap = hit.getSource();
			sourceMap["_id"] = hit.getId();
			resultSet.aaData.add(sourceMap);
		}

		resultSet.iTotalRecords = searchResponse.hits.getTotalHits();
		resultSet.iTotalDisplayRecords = searchResponse.hits.getTotalHits();
